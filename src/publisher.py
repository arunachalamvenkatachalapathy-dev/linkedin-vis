...
def upload_document(pdf_path: str, author_urn: str) -> str:
    log.info(f"Uploading document PDF to LinkedIn: {pdf_path} ({os.path.getsize(pdf_path)} bytes)")
    init_resp = requests.post(
    ...
    upload_resp.raise_for_status()
    log.info(f"Document upload successful: {document_urn}")
    return document_urn

def create_post(post_text: str, image_urn: str, author_urn: str, document_urn: str = "") -> dict:
    body = {
        "author": author_urn,
    ...
    if document_urn:
        body["content"] = {"media": {"id": document_urn, "title": "EcoPulse Carousel Insights"}}
    elif image_urn:
...
def publish_to_linkedin(post_text: str, media_path: str = "", post_format: str = "single_image") -> dict:
    token = os.environ.get("LINKEDIN_ACCESS_TOKEN", "").strip()
    ...
    if dry_run or not token or not author_urn:
        ...
        log.info(f"Post Format: {post_format}")
        log.info(f"Post Preview (first 200 chars):\n{post_text[:200]}...")
        if media_path:
            log.info(f"Media File Generated: {media_path}")
    ...
    if media_path and os.path.exists(media_path):
        try:
            if post_format == "carousel" or media_path.endswith(".pdf"):
                document_urn = upload_document(media_path, author_urn)
            else:
                image_urn = upload_image(media_path, author_urn)
        except Exception as e:
            log.warning(f"Failed to upload media to LinkedIn: {e}. Publishing text-only post.")

    result = create_post(post_text, image_urn, author_urn, document_urn=document_urn)
